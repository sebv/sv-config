#!/bin/bash
# install janus git://github.com/carlhuda/janus.git
# install PeepOpen https://peepcode.com/products/peepopen
cp snipmate/html.snippets ~/.vim/janus/vim/tools/snipmate-snippets/snippets/html.snippets
cp syntastic/html.vim ~/.vim/janus/vim/tools/syntastic/syntax_checkers/html.vim
cp syntastic/sass.vim ~/.vim/janus/vim/tools/syntastic/syntax_checkers/sass.vim
cp syntastic/scss.vim ~/.vim/janus/vim/tools/syntastic/syntax_checkers/scss.vim
cp vim/.vimrc.before ~
cp vim/.vimrc.after ~

